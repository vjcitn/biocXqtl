...
```{r lkpla,message=FALSE}
library(PlinkMatrix)
p19 = PlinkMatrix(file.path(tdir, "chr19_limsamples_maf10"))
p19
```

## Using python `bed-reader`

We can make targeted reads from the resource when
bed-reader is available.  At present we skip this to
simplify matters using the package.

```{r dopy, message=FALSE,eval=FALSE}
library(reticulate)
